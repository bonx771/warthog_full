...
class GPSNode:
    ...
    def __init__(self):
        ...
        rospy.init_node("stm32_gps_node")

        # Publisher GPS parsed
    ...
    def serial_reader(self):
        ...
        while not rospy.is_shutdown():
            ...
            while len(buffer) >= FRAME_LEN:
                ...
                lat = struct.unpack_from("<i", frame, 5)[0] * 1e-7
                lon = struct.unpack_from("<i", frame, 9)[0] * 1e-7
                alt = struct.unpack_from("<i", frame, 13)[0]
                
                sat = frame[17]
                mean_accuracy = frame[18]
                rtk = frame[19]
                fix = frame[20]
                ...
                if abs(lat) <= 90 and abs(lon) <= 180:
                    ...
                    rospy.loginfo(
                        f"GPS lat={lat} lon={lon} alt={alt} sat={sat} fix={fix}"
                    )
